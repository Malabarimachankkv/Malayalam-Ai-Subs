...
async function translateEntries(apiKey, entries, { onProgress } = {}) {
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    if (onProgress) {
      // Snapshot = finished Malayalam so far + still-untranslated English for
      // the rest of the movie. Lets the caller serve a fully-synced subtitle
      // file at every point in time instead of an all-or-nothing result.
      const snapshot = translated.concat(entries.slice(translated.length));
      onProgress(i + 1, chunks.length, snapshot);
    }
  }
