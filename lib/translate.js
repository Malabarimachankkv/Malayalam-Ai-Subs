...
// buildPrompt() flattens a multi-line cue's internal line break into " / "
// so each entry stays on one numbered line in the prompt (required for
// Gemini to return the right number of array entries back). Gemini
// naturally echoes that same " / " back in its translation instead of
// treating it as a formatting device, so it has to be converted back to a
// real line break here — otherwise a two-line cue like "Line one\nLine two"
// comes back as a single line reading "വരി ഒന്ന് / വരി രണ്ട്" instead of
// actually wrapping onto two lines the way the original did.
...
async function translateEntries(apiKey, entries, { onProgress } = {}) {
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    chunk.forEach((e, idx) => {
      translated.push({ ...e, text: restoreLineBreaks(result[idx]) });
    });
