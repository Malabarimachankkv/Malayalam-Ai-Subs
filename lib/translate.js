...
// Real free-tier limits vary by account/project tier and Google changes them
// over time - hardcoding a fixed interval means either wasting time (guessed
// too conservative) or still hitting 429s (guessed too optimistic). Instead,
// start optimistic and adapt per key: any time a call gets rate-limited,
// adopt Google's own suggested retry delay as the new pacing interval for
// that key going forward, so it self-corrects to each user's real limit.
const DEFAULT_INTERVAL_MS = 4000;
const stateByKey = new Map(); // apiKey -> { intervalMs, lastCallAt }

function getState(apiKey) {
  if (!stateByKey.has(apiKey)) stateByKey.set(apiKey, { intervalMs: DEFAULT_INTERVAL_MS, lastCallAt: 0 });
  return stateByKey.get(apiKey);
}
...
// Called after a 429 - permanently widens this key's pacing interval to at
// least what Google says is needed, so subsequent chunks (this movie or any
// other under the same key) stop hammering into the same wall.
function learnFromRateLimit(apiKey, retryDelayMs) {
  const state = getState(apiKey);
  if (retryDelayMs && retryDelayMs > state.intervalMs) {
    console.log(`Adjusting pacing for this key: ${state.intervalMs}ms -> ${retryDelayMs}ms (Gemini's own guidance)`);
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    for (let attempt = 0; attempt < 4 && !result; attempt++) {
      try {
        ...
        lastErr = e;
        // On a 429, adopt Google's suggested delay as this key's new pacing
        // going forward (not just a one-off wait), so later chunks/movies
        // under the same key stop hitting the same wall repeatedly.
        if (e.status === 429 && e.retryDelayMs) {
          learnFromRateLimit(apiKey, e.retryDelayMs);
